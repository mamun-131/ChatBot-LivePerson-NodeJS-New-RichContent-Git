...
function windowKit(options) {
  ...
  this.socket = {
    ...
    registerRequests: function () {
      _this.options.apiRequestTypes.forEach(function (reqType) {
        _this.socket[_this.socket.toFuncName(reqType)] = function (
          body,
          headers
        ) {
    ...
    authJWT: function () {
      return new Promise(function (res, rej) {
        return _this.lpUtils.getJWT(_this.options.account).then(function (jwt) {
          return _this.lpUtils
            .getDomain(_this.options.account, "idp")
            .then(function (idpDomain) {
              return $.ajax({
                url:
                  "https://" +
                  idpDomain +
  ...
  this.socketOpened = function (socket) {
    ...
    socket.onNotification(
      socket.onNotificationType("MessagingEvent"),
      function (body) {
        body.changes.forEach(function (change_detail) {
          var direction =
            change_detail.originatorId == _this.participantId
              ? "sent"
              : "received";
    ...
    socket
      ...
      .then(function (resp) {
        var convId;
        socket.onNotification(
          _this.socket.subscriptionID(resp.body.subscriptionId),
  ...
  this.setInteractions = function (convId) {
    ...
    _this.sendMessageClose = function () {
      // message = message.trim();
  ...
  this.conversationNotification = function (socket, notificationBody) {
    notificationBody.changes.forEach(function (change) {
      if (change.type === "UPSERT") {
        ...
        if (
          change.result.conversationDetails.skillId != _this.options.skillId
        ) {
    ...
    if (_this.options.campaignId && _this.options.engagementId) {
      convBody = {
        campaignInfo: {
          ...
          engagementId: _this.options.engagementId,
        },
      };
